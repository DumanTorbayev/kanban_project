...
import { getBoardTimeEntries } from "@/entities/time-entry/api/get-board-time-entries";
...
const BoardPage = async ({ params, searchParams }: Props) => {
  ...
  const [
    ...
    getBoardTimeEntries(supabase, boardId),
  ]);
    ...
    <main className="min-h-svh bg-muted/30 p-6">
      <AppContainer>
        <BoardHeader
          ...
          members={boardMembersResult.data ?? []}
        />
        <KanbanBoard
          ...
          error={
            queryParams?.error ??
            kanbanResult.error?.message ??
            boardMembersResult.error?.message
          }
